...
async fn test_get_list_empty() -> Result<()> {
    ...
    let docs = pool.get_list("EmptyDocType", None, None, None, None).await?;
...
async fn test_get_list_with_results() -> Result<()> {
    ...
    let limited = pool.get_list("TestDocType", None, None, None, Some(2)).await?;
