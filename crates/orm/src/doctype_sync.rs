...
pub async fn sync_all(
    ...
    ensure_dynamic_fields(pool).await?;
...
async fn ensure_dynamic_fields(pool: &DatabasePool) -> Result<()> {
    create_dynamic_field_table(pool).await?;
    seed_dynamic_field_rules(pool).await?;

    let rules = pool
        .execute_sql(
            r#"SELECT * FROM "__kiff_dynamic_field" WHERE enabled = 1"#,
            vec![],
        )
        .await?;
...
async fn ensure_dynamic_field_client_scripts(pool: &DatabasePool) -> Result<()> {
    ...
    refresh: function(frm) {{
        ...
        pool.execute_sql(
            &sql,
            vec![val(name), val(doctype.clone()), val(script)],
        )
        .await?;
...
fn row_str(row: &std::collections::HashMap<String, serde_json::Value>, key: &str) -> String {
    row.get(key)
        .and_then(|v| v.as_str())
        .unwrap_or("")
        .to_string()
...
mod tests {
    ...
    async fn dynamic_fields_are_injected_and_client_script_generated() {
        ...
        create_metadata_tables(&pool).await.unwrap();
        pool.execute_sql(
        ...
        assert!(
            idx > 10,
            "dynamic field should be placed after user_type (idx 10), got {}",
            idx
        );
        ...
        assert!(script.contains("user_type"));
