(() => {
  ...
  frappe.format = function(value, df, options, doc) {
    ...
    if (df == null ? void 0 : df.parent) {
      const mask_fields = (_a = frappe.get_meta(df.parent)) == null ? void 0 : _a.masked_fields;
      mask_readonly = mask_fields == null ? void 0 : mask_fields.includes(df.fieldname);
    }
    if (!df || mask_readonly)
      df = { fieldtype: "Data" };
  ...
  var Tab = class {
    constructor(layout, df, frm, tab_link_container, tabs_content) {
      ...
      this.df = df || {};
      this.frm = frm;
      this.doctype = (layout == null ? void 0 : layout.is_child_table) ? layout.doctype : (_b = (_a = this.frm) == null ? void 0 : _a.doctype) != null ? _b : this.df.parent;
    ...
    set_active() {
      ...
      if ((_a = this.layout) == null ? void 0 : _a.tabs) {
      ...
      if ((_b = this.layout) == null ? void 0 : _b.grid_row_form) {
        (_d = (_c = this.layout.grid_row_form).set_active_tab) == null ? void 0 : _d.call(_c, this);
      } else {
        (_f = (_e = this.frm) == null ? void 0 : _e.set_active_tab) == null ? void 0 : _f.call(_e, this);
      }
    }
    is_active() {
  ...
  var Column = class {
    ...
    resize_all_columns() {
      ...
      all_columns.each(function() {
        ...
        $col.removeClass().addClass("form-column").addClass("col-sm-" + colspan);
        if (is_hidden) {
  ...
  frappe.ui.form.Layout = class Layout {
    ...
    show_message(html, color, permanent = false) {
      ...
      const block_color = color && ["yellow", "blue", "red", "green", "orange", "white"].includes(color) ? color : "blue";
    ...
    refresh_dependency() {
      ...
      for (const f of fields) {
        ...
        if (f.df.fieldtype === "Table") {
          for (const row of ((_a = f.grid) == null ? void 0 : _a.grid_rows) || []) {
            row == null ? void 0 : row.refresh_dependency();
          }
        }
      }
    }
        ...
        function(results) {
          ...
          if (results.length) {
            for (const v of results) {
              var row = $(
                repl(
                  '<div class="row link-select-row">						<div class="col-xs-4 text-break">							<b><a href="#">%(name)s</a></b></div>						<div class="col-xs-8 text-break">							<span class="text-muted">%(values)s</span></div>						</div>',
                  {
                    name: v[0],
  ...
  frappe.ui.form.MultiSelectDialog = class MultiSelectDialog {
    ...
    setup_results() {
      ...
      this.$wrapper = this.dialog.fields_dict.results_area.$wrapper.append(`<div class="results mt-2 mb-3"
    ...
    bind_events() {
      ...
      this.$parent.find(".input-with-feedback").on("awesomplete-selectcomplete", refresh_results);
    ...
    make_list_row(result = {}) {
      ...
      this.get_datatable_columns().forEach(function(column) {
        contents += `<div class="list-item__content ellipsis">
          ...
          result[column] || ""
        )}">${__(result[column] || "")}</span>` : `<a href="${"/desk/" + frappe.router.slug(me.doctype) + "/" + result[column] || ""}" class="list-id ellipsis" title="${__(result[column] || "")}">
							${__(result[column] || "")}</a>`}
			</div>`;
      });
    ...
    empty_list() {
      let checked = [...this.selected_fields].map((name) => __spreadProps(__spreadValues({}, this.selected_items[name] || { name }), {
        checked: true
      }));
    ...
    get_filters_from_setters() {
      ...
      if ($.isArray(this.setters)) {
        for (let df of this.setters) {
          let value = me.dialog.fields_dict[df.fieldname].get_value() || df.default || void 0;
  ...
  frappe.ui.Dialog = class Dialog extends frappe.ui.FieldGroup {
    ...
    make() {
      ...
      if (this.centered)
        $(this.wrapper).addClass("modal-dialog-centered");
      if (this.size == "small")
      ...
      me.$wrapper[0].addEventListener(
        ...
        function(e) {
          if (e.key === "Escape" || e.keyCode === 27) {
            me._awesomplete_was_open = me.display && !!me.$wrapper.find(".awesomplete input[aria-expanded='true']").length;
          }
        },
        true
      );
    ...
    handle_focus() {
      var _a;
      const me = this;
      if ((_a = frappe.get_route) == null ? void 0 : _a.call(frappe)) {
        if (frappe.get_route()[0] == "Form") {
          if (!me.last_focus)
            me.last_focus = document.activeElement;
        }
...
//# sourceMappingURL=dialog.bundle.MZMF5SQK.js.map
