(() => {
  ...
  frappe.views.ReportView = class ReportView extends frappe.views.ListView {
    ...
    set_link_title_field_value() {
      var _a, _b, _c, _d, _e;
      let rows = (_b = (_a = this.datatable) == null ? void 0 : _a.datamanager) == null ? void 0 : _b.rows;
      let link_col_indices = (_e = (_d = (_c = this.datatable) == null ? void 0 : _c.datamanager) == null ? void 0 : _d.columns) == null ? void 0 : _e.filter((c) => {
        var _a2;
        return ((_a2 = c.docfield) == null ? void 0 : _a2.fieldtype) === "Link";
      }).map((c) => c.colIndex);
      Object.keys(this.link_title_doctype_fields).forEach(async (key) => {
        ...
        document.querySelectorAll(`a[data-name="${key}"]`).forEach((el) => {
          if (el.textContent === link_title)
            return;
          el.textContent = link_title;
          $(el).closest(".dt-cell__content").attr("title", link_title);
        });
        if ((rows == null ? void 0 : rows.length) && (link_col_indices == null ? void 0 : link_col_indices.length)) {
          for (let row of rows) {
            for (let ci of link_col_indices) {
              let cell = row[ci];
              if ((cell == null ? void 0 : cell.content) === key && cell.html) {
                cell.html = null;
              }
            }
          }
    ...
    update_count_for_inline_filter() {
      if (!this.datatable)
        return;
      const has_active_filters = this.datatable.columnmanager ? Object.keys(this.datatable.columnmanager.getAppliedFilters()).length > 0 : false;
          ...
          if (Array.isArray(row)) {
            doc = row.reduce((acc, curr) => {
              if (!curr.column.docfield)
                return acc;
              if (curr.column.docfield.fieldtype == "Link" && frappe.boot.link_title_doctypes.includes(curr.column.docfield.options)) {
        ...
        },
        {
          ...
          action: () => {
            ...
            const $bulk = $(`
					...
					`);
            const toggleAll = (checked) => d.$wrapper.find(":checkbox").prop("checked", checked).trigger("change");
  ...
  frappe.views.QueryReport = class QueryReport extends frappe.views.BaseList {
    ...
    setup_filters() {
      ...
      this.filters = [];
      if (this.report_settings.separate_check_filters)
    ...
    refresh(have_filters_changed) {
      ...
      return new Promise((resolve) => {
        var _a;
        const js_filters = (((_a = frappe.query_reports[this.report_name]) == null ? void 0 : _a.filters) || []).filter(
        ...
        const check_if_report_is_stale = () => {
          let generated_at = this.prepared_report && this.prepared_report_document ? this.prepared_report_document.report_end_time : this.refreshed_at;
        ...
        this.stale_report_interval = setInterval(check_if_report_is_stale, 6e4);
        ...
        if (data.message && !data.prepared_report)
          this.show_report_message(data.message);
    ...
    get_queued_prepared_reports_warning_message(reports) {
      ...
      let get_item_html = (item) => `<a class="underline" href="/desk/prepared-report/${item.name}">${item.name}</a>`;
    ...
    generate_background_report() {
      ...
      if (!missing_mandatory.length) {
        ...
        return new Promise(
          ...
          let alert_message = `<a href='/desk/prepared-report/${data.name}'>` + __("Report initiated, click to view status") + `</a>`;
    ...
    render_datatable() {
      ...
      if ((_a = this.report_doc) == null ? void 0 : _a.ref_doctype) {
        columns = this.update_masked_fields_in_columns(columns, (_b = this.report_doc) == null ? void 0 : _b.ref_doctype);
      }
    ...
    update_masked_fields_in_columns(columns) {
      var _a;
      const masked_fields = frappe.get_meta((_a = this.report_doc) == null ? void 0 : _a.ref_doctype).masked_fields;
    ...
    set_filter_value(fieldname, value) {
      ...
      Object.keys(field_value_map).forEach((fieldname2, i, arr) => {
        ...
        const filter = this.get_filter(fieldname2);
        if (filter) {
          filter.set_value(value2);
        }
      });
    }
    set_breadcrumbs() {
    ...
    get_validated_visible_indexes() {
      ...
      if (!(visible_idx == null ? void 0 : visible_idx.length)) {
        frappe.throw({
    ...
    async print_report(print_settings) {
      ...
      frappe.render_grid({
        ...
        subtitle: (print_settings == null ? void 0 : print_settings.include_filters) ? filters_html : null,
        print_settings,
        landscape,
        filters: this.get_filter_values(),
    ...
    async get_custom_format(print_settings) {
      ...
      } else if (!((_a = print_settings.columns) == null ? void 0 : _a.length) && typeof this.report_settings.get_pdf_format === "function") {
    ...
    get_print_template(print_settings, custom_format) {
      var _a;
      return ((_a = print_settings.columns) == null ? void 0 : _a.length) || !custom_format ? "print_grid" : custom_format;
    }
    ...
    async render_report_letterhead(print_settings) {
      if (!print_settings.with_letter_head || !print_settings.letter_head_name)
        return;
    ...
    export_report() {
      ...
      if (this.report_settings.export_hidden_cols) {
        const hidden_fields = /* @__PURE__ */ new Set();
      ...
      this.export_dialog = frappe.report_utils.get_export_dialog(
        ...
        ({
          ...
          const applied_filters2 = this.get_applied_filters(filters);
          if (this.prepared_report_name) {
            filters.prepared_report_name = this.prepared_report_name;
          }
          const ignore_visible_idx = visible_idx.length === this.data.length - (this.raw_data.add_total_row ? 1 : 0);
          visible_idx = ignore_visible_idx ? [] : visible_idx;
          const args = {
            cmd: "frappe.desk.query_report.export_query",
            report_name: this.report_name,
            custom_columns: ((_a = this.custom_columns) == null ? void 0 : _a.length) ? this.custom_columns : [],
            file_format_type: file_format,
            filters,
            applied_filters: applied_filters2,
            visible_idx,
            ignore_visible_idx,
            csv_delimiter,
            csv_quoting,
    ...
    get_data_for_print() {
      ...
      if (this.raw_data.add_total_row && !this.report_settings.tree) {
        ...
        if (!(totalRow == null ? void 0 : totalRow.currency) && ((_a = rows[0]) == null ? void 0 : _a.currency)) {
    ...
    get_columns_for_print(print_settings, custom_format) {
      var _a;
      let columns = [];
      if (print_settings && ((_a = print_settings.columns) == null ? void 0 : _a.length)) {
              ...
              fields: [
                {
                  ...
                  change: () => {
                    ...
                    frappe.model.with_doctype(doctype, () => {
                      ...
                      (_a2 = d.get_field("field")) == null ? void 0 : _a2.set_data(options);
                      d.set_value("field", "");
                    });
                  }
                },
              ...
              primary_action: (values) => {
                ...
                frappe.call({
                  ...
                  args: {
                    ...
                    doctype: values.doctype,
                    names: Array.from(
                      this.doctype_field_map[values.doctype][values.fieldname].names
                    )
                  },
                  callback: (r) => {
    ...
    get_linked_doctypes() {
      ...
      doctypes.forEach((doc) => {
        ...
        this.doctype_field_map[doc.doctype][doc.fieldname] = { names: /* @__PURE__ */ new Set() };
      });
      this.data.forEach((row) => {
        doctypes.forEach((doc) => {
  ...
  // frappe-html:/Users/johannnefdt/RustroverProjects/open_frappe/apps/frappe/frappe/public/js/frappe/views/reports/print_grid.html
...
`;
  ...
  // frappe-html:/Users/johannnefdt/RustroverProjects/open_frappe/apps/frappe/frappe/public/js/frappe/views/reports/print_tree.html
...
`;
  ...
  // frappe-html:/Users/johannnefdt/RustroverProjects/open_frappe/apps/frappe/frappe/public/js/frappe/ui/group_by/group_by.html
  ...
  frappe.report_utils = {
    make_chart_options: function(columns, raw_data, { y_fields, x_field, chart_type, colors, height }) {
      ...
      function get_translated_column_label(fieldname) {
        var _a;
        let column = columns.find((column2) => column2.fieldname === fieldname);
        return (_a = column == null ? void 0 : column.label) != null ? _a : __(frappe.model.unscrub(fieldname));
      }
    },
...
//# sourceMappingURL=report.bundle.7KEGB6OG.js.map
